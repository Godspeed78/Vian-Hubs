...
class ManageShopWorkers extends Component {
	...
	handleCreateShopWorker = () => {
		...
		Endpoint.createShopWorker(payload)
			...
			.catch((error) => {
				this.setState({errorMsg:true, spinner:false, content_message:"Oops! Unable to process request. Please check that you have an active internet connection"})
				setTimeout(() => {
					this.setState({errorMsg:false})
				},3000)
			});
	...
	getShopWorkers = () => {
		Endpoint.getShopWorkers()
			.then(res => {
				...
				console.log(this.state.shopWorkers, "Shop Workers")
			})
	};
												...
												this.state.shopWorkers.map((a, b) => {
													return (
														<tr>
															...
															<td>{a.user.phone}</td>
															<td>
																{a.worker_type_id == worker.cashier
																	? "Cashier"
																	: a.worker_type_id == worker.attendant
																		? "Attendant"
																		: "Admin"}
															</td>
															<td>
